...
while IFS=$'\t' read -r snapshot_id source_resource_tag all_tags_json; do
    # Debug: Print the values read from jq
    echo "DEBUG: snapshot_id=$snapshot_id, source_resource_tag=$source_resource_tag, all_tags_json=$all_tags_json"
    ...
    .SnapshotId,
    (.SourceResource // ""),
    (if .AllTags then .AllTags else [] end) | @json
] | @tsv')
